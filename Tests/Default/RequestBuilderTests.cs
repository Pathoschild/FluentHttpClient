...
namespace Pathoschild.Http.Tests.Default
{
	...
	public class RequestBuilderTests
	{
		...
		*********/
		...
		[Test(Description = "Ensure that Retrieve returns a response with the expected initial state and does not incorrectly alter request state.")]
		[TestCase("DELETE")]
		[TestCase("GET")]
		[TestCase("HEAD")]
		[TestCase("PUT")]
		...
		[TestCase("POST")]
		[TestCase("TRACE")]
		public void Retrieve(string methodName)
		{
			// execute
			...
			this.AssertEqual(response.Request, methodName);
		}
		...
		** Protected methods
		*********/
		/// <summary>Construct an <see cref="IRequestBuilder"/> instance and assert that its initial state is valid.</summary>
		...
		protected IRequestBuilder ConstructRequest(string methodName, string uri = "http://example.org/", bool inconclusiveOnFailure = true)
		{
			try
			{
				// set up
				...
				// execute
				IRequestBuilder request = new RequestBuilder(message, new MediaTypeFormatterCollection(), r => new Task<HttpResponseMessage>(() => null));
